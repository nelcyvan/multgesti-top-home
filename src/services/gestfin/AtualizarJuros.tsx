// Serviço para atualizar juros (TXPERM) de um lançamento por RECNUM

export interface AtualizarJurosPayload {
  recnum: number;
  juros: number; // valor em reais, ex.: 10.5
}

export interface AtualizarJurosResponse {
  ok?: boolean;
  rowsAffected?: number;
  error?: string;
}
...
// Executa no backend: UPDATE PCLANC SET TXPERM = :juros WHERE RECNUM = :recnum
export async function atualizarJuros(
  payload: AtualizarJurosPayload
): Promise<AtualizarJurosResponse> {
  const baseApi = resolveBaseApi();
  const url = `${baseApi}/gestfin/atualizar-juros`;
  ...
  if (!resp.ok) {
    const message = (data as any)?.error || `Erro ao atualizar juros: ${resp.status}`;
