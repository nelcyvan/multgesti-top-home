...
const VisualizarModal: React.FC<VisualizarModalProps> = ({ isOpen, onClose, dadosLinha }) => {
  ...
  const renderHistoricoOFX = (val?: string) => {
    if (!val) return null;
        ...
        <div className="modal-dialog modal-md modal-dialog-centered" role="document" style={{ maxWidth: "40vw" }}>
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" style={{ fontSize: "0.9rem" }}>Visualizar Lançamento</h5>
              ...
              <table className="table table-striped table-bordered table-hover table-sm" style={{ fontSize: "0.7rem" }}>
                <tbody>
                  ...
                  <tr>
                    ...
                    <td style={{ whiteSpace: 'normal', wordBreak: 'break-word' }}>{renderHistoricoOFX(dadosLinha?.HISTORICO)}</td>
                  </tr>
                  <tr>
                    <td><strong>Valor Transação:</strong></td>
                    <td className={toNumber(dadosLinha?.VALOR_TRANSACAO) < 0 ? 'text-danger' : 'text-success'}>{renderWithCommaBreaks(dadosLinha?.VALOR_TRANSACAO)}</td>
                  </tr>
                  <tr>
                    <td><strong>Banco/Filial:</strong></td>
                    <td>{renderWithCommaBreaks(dadosLinha?.NOME_BANCO_FILIAL)}</td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    ...
                    <td style={{ whiteSpace: 'normal', wordBreak: 'break-word' }}>{renderWithCommaBreaks(dadosLinha?.FORNECEDOR)}</td>
                  </tr>
                  <tr>
                    <td><strong>Histórico Duplicata:</strong></td>
                    <td style={{ whiteSpace: 'normal', wordBreak: 'break-word' }}>{renderWithCommaBreaks(dadosLinha?.HISTORICO_DUPLICATA)}</td>
                  </tr>
                  <tr>
                    <td><strong>NFs:</strong></td>
                    <td>{renderWithCommaBreaks(dadosLinha?.NFSERVICO_STATUS)}</td>
                  </tr>
                  <tr>
                    <td><strong>Nº Nota:</strong></td>
                    <td>{renderWithCommaBreaks(dadosLinha?.NUMNOTA)}</td>
                  </tr>
                  <tr>
                    <td><strong>Valor Interno:</strong></td>
                    <td className={toNumber(dadosLinha?.VALOR_LANCAMENTO_INTERNO) < 0 ? 'text-danger' : 'text-success'}>{renderWithCommaBreaks(dadosLinha?.VALOR_LANCAMENTO_INTERNO)}</td>
                  </tr>
                  <tr>
