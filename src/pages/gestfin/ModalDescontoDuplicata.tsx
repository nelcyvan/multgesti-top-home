...
import { atualizarDesconto } from "../../services/gestfin/AtualizarDesconto";

interface ModalDescontoDuplicataProps {
  isOpen: boolean;
  onClose: () => void;
  ...
  onSuccess?: (novoDesconto: number) => void;
}
...
const ModalDescontoDuplicata: React.FC<ModalDescontoDuplicataProps> = ({ isOpen, onClose, duplicata, valorOfx, resumoOfx, onSuccess }) => {
  ...
  const [descontoInput, setDescontoInput] = useState<string>(descontoAtual.toString());
  const [saving, setSaving] = useState<boolean>(false);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);

  const handleAplicar = async () => {
    setErrorMsg(null);
    const valor = toNumber(descontoInput);
    if (valor < 0) {
      setErrorMsg("Desconto não pode ser negativo.");
      return;
    }
    setSaving(true);
    try {
      await atualizarDesconto({ recnum: Number(duplicata.RECNUM), desconto: valor });
      onSuccess?.(valor);
      onClose();
    } catch (e: any) {
      setErrorMsg(String(e?.message || e || "Falha ao atualizar desconto"));
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="modal-dialog modal-md modal-dialog-centered" role="document">
          <div className="modal-content">
            <div className="modal-header py-2">
              <h5 className="modal-title" style={{ fontSize: "0.9rem" }}>Atualizar Desconto</h5>
            ...
            <div className="modal-body" style={{ fontSize: "0.75rem", lineHeight: 1.1 }}>
              ...
              <div className="mb-3">
                <label className="form-label">Novo Desconto</label>
...
export default ModalDescontoDuplicata;