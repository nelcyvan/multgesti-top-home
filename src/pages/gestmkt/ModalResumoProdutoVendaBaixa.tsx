...
interface ModalResumoProdutoVendaBaixaProps {
  isOpen: boolean;
  onClose: () => void;
  row: ProdutoVendaBaixaRow;
  onSuccess?: () => void;
}

const ModalResumoProdutoVendaBaixa: React.FC<ModalResumoProdutoVendaBaixaProps> = ({ isOpen, onClose, row, onSuccess }) => {
  ...
  const confirmar = () => {
    ...
    registrarPromocao(payload)
      .then((resp) => {
        ...
        // Fecha primeiro, depois aciona callback de sucesso para recarregar dados no pai
        onClose();
        try { onSuccess?.(); } catch {}
      })
      .catch((err) => {
                  ...
                  <div className="card border-0 bg-light">
                    <div className="card-body">
                      <div style={{ fontSize: "0.8rem" }}>
                        ...
                        <div className="mb-2"><strong>Últ. Saída:</strong> {String(row.NOVA_DTULTSAIDA ?? '')}</div>
                        <div className="mb-2"><strong>Custo:</strong> {Number(row.CUSTOULTENT ?? 0).toFixed(2)}</div>
                        <div className="mb-2"><strong>Venda:</strong> {Number(row.PVENDA ?? 0).toFixed(2)}</div>
                        <div className="mb-0"><strong>Vendido no período:</strong> {Number(row.VENDA_TOTAL ?? 0)}</div>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="row g-3 align-items-end">
                    ...
                    <div className="col-12 col-md-3">
                      ...
                      >
                        ...
                        <option value="1">Messejna</option>
...
export default ModalResumoProdutoVendaBaixa;