...
const ModalBuscaAvancadaDescricaoGestpro: React.FC<Props> = ({ codFilialSel, descricaoInicial, onCancelar }) => {
  ...
  const podeBuscar = useMemo(() => {
    return !!desc.trim() && ["1","2","3","4"].includes(String(codFilialSel || ""));
  }, [desc, codFilialSel]);
  ...
  const executarBusca = async () => {
    ...
    if (!["1","2","3","4"].includes(String(codFilialSel || ""))) {
      ...
      <div className="modal-body" style={{ fontSize: "0.75rem", lineHeight: 1.1, minHeight: "58vh" }}>
        <div className="row g-2 align-items-end">
          <div className="col-12 col-md-7">
            <label htmlFor="busca-desc-gestpro" className="form-label mb-1">Descrição</label>
            <input
              id="busca-desc-gestpro"
              type="text"
        ...
        <div className="mt-2" style={{ maxHeight: "50vh", overflowY: "auto" }}>
          <table className="table table-sm table-hover" style={{ fontSize: "0.72rem", tableLayout: "fixed" }}>
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  <td colSpan={7} className="text-center text-muted">Nenhum item listado</td>
                </tr>
              ) : (
                rows.map((r, idx) => (
...
export default ModalBuscaAvancadaDescricaoGestpro;