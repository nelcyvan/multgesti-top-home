...
const AguardandoFornecedorPendenciasCard: React.FC<Props> = ({ pendencias, bodyHeight }) => {
  const pendenciasFornecedor = useMemo(
    () => pendencias.filter(p => String(p.LOG2) === "10"),
    [pendencias]
  );

  const groups = useMemo(() => Array.from(groupPendencias(pendenciasFornecedor)), [pendenciasFornecedor]);

  return (
    <div className="card border-0 bg-light shadow-lg h-100" style={{ borderLeft: "4px solid #fd7e14" }}>
      <div className="card-header border-0 bg-transparent py-1 d-flex justify-content-between align-items-center">
        ...
        <span className="badge bg-warning text-dark rounded-pill px-2" style={{ fontSize: "0.75rem" }}>
          {pendenciasFornecedor.length}
        </span>
      </div>
      <div className="card-body p-0" style={{ height: bodyHeight ?? "calc(50vh - 100px)", overflowY: "auto" }}>
        <div className="table-responsive">
          {pendenciasFornecedor.length === 0 ? (
            <div className="text-center py-2 text-muted" style={{ fontSize: "0.85rem" }}>Nenhum item de Falta de Produtos Entrega/Retira.</div>
          ) : (
                ...
                <div key={numped} className="card mb-3 mx-2 border shadow-sm">
                  ...
                  <div className="card-body p-0">
                    <table className="table table-hover table-sm mb-0" style={{ fontSize: "0.75rem" }}>
                      ...
                      <tbody>
                        {items.map((p, idx) => (
                          <tr key={`faltaprod-${p.NUMPED}-${p.CODPROD}-${idx}`}>
...
export default AguardandoFornecedorPendenciasCard;
