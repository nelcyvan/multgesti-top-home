export default function registerComissoesPorFreteMesAnteriorEmAbertoTotal(router, { oracledb }) {
  router.get("/comissoes-por-frete-mes-anterior-em-aberto-total", async (req, res) => {
    console.log("[GestPRO] Acessando /api/gestpro/comissoes-por-frete-mes-anterior-em-aberto-total");
    let conn;
    try {
      ...
      const now = new Date();
      const data1Default = new Date(now.getFullYear(), now.getMonth() - 1, 1);
      const data2Default = new Date(now.getFullYear(), now.getMonth(), 0);
      ...
      const data1Param =
        typeof data1Query === "string" && data1Query.trim() ? new Date(`${data1Query}T00:00:00`) : data1Default;
      const data2Param =
        typeof data2Query === "string" && data2Query.trim() ? new Date(`${data2Query}T00:00:00`) : data2Default;
      ...
      const sql = `
        ...
        base AS (
          ...
          WHERE PCPREST.NUMTRANSVENDA = PCNFSAID.NUMTRANSVENDA
            ...
            AND PCPREST.DTEMISSAO >= P.DATA1
            AND PCPREST.DTEMISSAO < (P.DATA2 + 1)
        ),
        ranked AS (
          SELECT
            base.*,
      ...
      console.error("Erro ao buscar Fretes em Aberto (Mês Anterior - Total):", err);
  ...
  console.log("[GestPRO] Registrada rota GET /api/gestpro/comissoes-por-frete-mes-anterior-em-aberto-total");
}
