export default function registerComissoesPorFreteMesAtualEmAbertoTotal(router, { oracledb }) {
  router.get("/comissoes-por-frete-mes-atual-em-aberto-total", async (req, res) => {
    console.log("[GestPRO] Acessando /api/gestpro/comissoes-por-frete-mes-atual-em-aberto-total");
    let conn;
    try {
      ...
      const now = new Date();
      const data1Default = new Date(now.getFullYear(), now.getMonth(), 1);
      const data2Default = new Date(now.getFullYear(), now.getMonth() + 1, 0);
      ...
      console.error("Erro ao buscar Fretes em Aberto (Mês Atual - Total):", err);
  ...
  console.log("[GestPRO] Registrada rota GET /api/gestpro/comissoes-por-frete-mes-atual-em-aberto-total");
}
