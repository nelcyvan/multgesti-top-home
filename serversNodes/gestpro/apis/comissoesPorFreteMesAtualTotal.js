export default function registerComissoesPorFreteMesAtualTotal(router, { oracledb }) {
  router.get("/comissoes-por-frete-mes-atual", async (req, res) => {
    console.log("[GestPRO] Acessando /api/gestpro/comissoes-por-frete-mes-atual");
    let conn;
    try {
      ...
      const now = new Date();
      const data1 = new Date(now.getFullYear(), now.getMonth(), 1);
      const data2 = new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59);
      ...
      console.error("Erro ao buscar Comissões por Frete (Mês Atual):", err);
  ...
  console.log("[GestPRO] Registrada rota GET /api/gestpro/comissoes-por-frete-mes-atual");
}
