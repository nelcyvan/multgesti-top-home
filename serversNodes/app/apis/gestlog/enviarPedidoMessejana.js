export default function registerEnviarPedidoMessejana(router, { oracledb }) {
  router.post("/enviar-messejana", async (req, res) => {
    const { numped, status, usuario } = req.body || {};

    const numpedNum = Number(numped);
    const statusNum = Number(status ?? 20);
    const usuarioStr = String(usuario || "").trim();

    if (!Number.isFinite(numpedNum) || !Number.isFinite(statusNum) || !usuarioStr) {
      return res.status(400).json({ message: "Parâmetros inválidos: informe 'numped' numérico, 'status' numérico (opcional) e 'usuario'" });
    }
